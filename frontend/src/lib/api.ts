...
export const storeSettingsApi = {
  get: () => api.get<StoreSettings>('/store-settings'),
  update: (data: UpdateStoreSettingsInput) => api.put<StoreSettings>('/store-settings', data),
};
...
export const authApi = {
  ...
  forgotPassword: (phoneNumber: string) => api.post<{ message: string }>('/auth/forgot-password', { phoneNumber }),
...
// Add request interceptor to include token for both api and uploadApiInstance
...
uploadApiInstance.interceptors.request.use(addAuthToken, (error) => Promise.reject(error));
